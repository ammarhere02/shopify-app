...
| `app.products.$id.tsx` | `/app/products/:id` | Editor. `:id` is the LOCAL product id, looked up with shopId (other shop → 404). Action: `intent=save` (validate → `saveEnrichment`) or `intent=remove`. Loader also prepares the AI description section: `loadAiConfig` (names and limits only, never the key; missing config → `configured: false`), a live Shopify read of the product's images (a failure becomes a banner, the badge editor still works), the last 10 generations and the latest one in full. Layout: compact header (first thumbnail + status badges), then `components/AiDescriptionSection`, which owns the two-column grid and receives the page's badge editor and collapsed product details as `asideTop`/`asideBottom` for its left column (a deleted product renders those two cards alone, so the editor exists exactly once). Page width stays `base`: the Polaris page's `aside` slot is only rendered at `inlineSize="base"`, so the layout uses an explicit `s-grid` instead of that slot. Save/Remove show `loading` on the clicked button; success is a toast (`shopify.toast.show`), failure a banner with field errors |
...
## UI
Polaris web components (`<s-page>`, `<s-section>`, `<s-text-field>`, `<s-table>`…), typed by `@shopify/polaris-types`. Form values are held in React state and sent with `fetcher.submit`; inputs use `onInput`/`onChange` with `e.currentTarget.value`. Internal links: `<s-link href="/app/...">`. Pages keep the default `inlineSize` ("base", the admin's normal content width; "large" stretches every field and drops the page `aside`). Responsive layouts use `s-grid` container-query values (`@container (inline-size > 640px) 1fr 1fr, 1fr`), and they only resolve inside an `s-query-container` ancestor (without one every responsive grid falls back to its last value, i.e. one column), so each page wraps its responsive content in one. Table headers use `listSlot`. The only custom CSS is the scoped tab strip in `AiDescriptionSection` (class prefix `eh-`). Success feedback is `shopify.toast.show` (App Bridge global, typed by `@shopify/app-bridge-types`); errors stay as `s-banner`. The button that was clicked gets `loading` (from `fetcher.formData.get("intent")`) and the others `disabled` while a request runs; typing never shows a loader.
