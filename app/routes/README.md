...
|---|---|---|
| `app.tsx` | layout for `/app/*` | `authenticate.admin`, `AppProvider`, `NavMenu` (Products = home, Sync), and `shopify.loading(...)` from `useNavigation` so the admin's own loading bar runs while a page loader is in flight. Add new admin pages to the NavMenu here |
