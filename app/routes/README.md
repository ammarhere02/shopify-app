...
|---|---|---|
| `app.tsx` | layout for `/app/*` | `authenticate.admin`, `AppProvider`, `NavMenu` (Products = home, Sync), and, from `useNavigation`, `shopify.loading(...)` for the admin's own loading bar plus a centred green spinner overlay (scoped CSS, appears after 150 ms) while a page loader is in flight. Add new admin pages to the NavMenu here |
| `app._index.tsx` | `/app` | `authenticate.admin`, then redirects to `/app/products` (query string kept). The catalogue is the home screen |
| `app.sync.tsx` | `/app/sync` | Sync tools, reached from the nav. Loader: shop info, product/variant counts, latest sync run. Action: `intent=sync|reconcile` → `startSyncRun` + `runProductSync` (runs inside the request, 60s budget). Sync now / Reconcile sit in the page action slots; the clicked one shows `loading`, both are disabled while a run is in flight; the result shows in the top-of-page `Notice` |
| `app.products._index.tsx` | `/app/products` | List. Filters from URL: `query`, `status`, `hasBadge`, `after` (keyset cursor = last local id). Unknown values are ignored. Loader also sends the allowed model names and, from ONE query (`latestJobStatusByProduct`), each product's latest AI generation as a badge (`aiStatusLabel`: Queued/Running/Failed while generation is incomplete, the review status once it succeeded, "Not generated" without a job). Table headers carry `listSlot`s so rows collapse to a list on narrow screens; `paginate` + `onNextPage` drive the cursor. Product titles are React Router `Link`s (client-side navigation), so opening a product shows the admin loading bar, the table's `loading` state and a spinner on the clicked row | Action `intent=generateBatch` (`productIds[]`, `merchantContext`, `model`, `idempotencyKey`): queues one AI generation per selected product (max 20) through `startBatchGeneration`; rate-limited with the admin write bucket; result banner lists skipped products |
| `app.products.$id.tsx` | `/app/products/:id` | Editor. `:id` is the LOCAL product id, looked up with shopId (other shop → 404). Action: `intent=save` (validate → `saveEnrichment`) or `intent=remove`. Loader also prepares the AI description section: `loadAiConfig` (names and limits only, never the key; missing config → `configured: false`), a live Shopify read of the product's images (a failure becomes a banner, the badge editor still works), the last 10 generations and the latest one in full. Layout: the page renders the top `Notice`, then `components/AiDescriptionSection`, which owns the whole workspace and receives the page's status badges, badge editor form and product details as `statusBadges` / `badgeEditor` / `productDetails` for its left panel (a deleted product renders the editor and details as plain sections instead, so the editor exists exactly once). Page width stays `base`: the Polaris page's `aside` slot is only rendered at `inlineSize="base"`, so the layout uses an explicit `s-grid` instead of that slot. Save/Remove show `loading` on the clicked button; the result shows in the top-of-page `Notice`, field errors also stay next to the fields |
...
| `webhooks.products.create.tsx` | POST `/webhooks/products/create` | `authenticate.webhook` → `handleProductUpdate` (same re-fetch path; an unknown local row is created; the receipt keeps topic `PRODUCTS_CREATE`) |
...
| `api.v1.description-generations.$jobId.tsx` | GET | One generation in the `serializeGeneration` shape (`generated`, `warnings`, `usage`, and `research`: status, reason, matched product, facts with `sourceUrl`, rejected facts, cited `sources`, search count; `null` for generations made before research). Other shop or malformed id → 404 |
...
| `api.v1.description-generations.batch.tsx` | POST | `Idempotency-Key` + `{ productIds: string[1..20] (Shopify ids), merchantContext?, model? }` → 202 `{ jobs: [{ productId, generationId, created }], skipped: [{ productId, reason }] }`. Jobs are QUEUED for the worker, nothing runs inline. 404 when any id is not this shop's; 422 malformed |
...
## UI
Polaris web components (`<s-page>`, `<s-section>`, `<s-text-field>`, `<s-table>`…), typed by `@shopify/polaris-types`. Form values are held in React state and sent with `fetcher.submit`; inputs use `onInput`/`onChange` with `e.currentTarget.value`. Internal links: `<s-link href="/app/...">`. Pages keep the default `inlineSize` ("base", the admin's normal content width; "large" stretches every field and drops the page `aside`). Responsive layouts: Polaris `@container` responsive grid values did NOT resolve in the embedded admin (every grid stayed one column), so multi-column layouts use plain `div`s with scoped CSS and native container queries (`container-type: inline-size` + `@container (min-width: …)`), see `AiDescriptionSection`. Simple wrapping rows use `s-grid` with `repeat(auto-fit, minmax(…))`. Table headers use `listSlot`. Custom CSS lives only in the scoped `eh-` rules of `AiDescriptionSection`. Action results show in one `components/Notice` banner at the top of the page: success fades out after 5 s, errors stay until closed. The button that was clicked gets `loading` (from `fetcher.formData.get("intent")`) and the others `disabled` while a request runs; typing never shows a loader.

## Known gaps
No admin page for API keys.
