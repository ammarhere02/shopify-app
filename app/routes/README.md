...
| `app.products.$id_.generation.tsx` | `/app/products/:id/generation` (resource route, no UI; the `_` keeps it out of the product page's layout) | Loader `?jobId=`: one generation of this shop AND this product, polled every 2s while QUEUED/RUNNING. Action intents: `generate`, `regenerate` (start the run without awaiting it), `saveDraft` (sanitized), `approve` (saves the editor's text first), `reject`, `reopen`. `GenerationError` and a missing OpenRouter configuration come back as `{ ok: false, message, errors }`, not as an error page |
