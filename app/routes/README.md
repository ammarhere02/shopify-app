...
|---|---|---|
| `app.tsx` | layout for `/app/*` | `authenticate.admin`, `AppProvider`, `NavMenu` (Products = home, Sync). Add new admin pages to the NavMenu here |
...
| `app.products._index.tsx` | `/app/products` | List. Filters from URL: `query`, `status`, `hasBadge`, `after` (keyset cursor = last local id). Unknown values are ignored. Loader also sends the allowed model names and, from ONE query (`latestJobStatusByProduct`), each product's latest AI generation as a badge (`aiStatusLabel`: Queued/Running/Failed while generation is incomplete, the review status once it succeeded, "Not generated" without a job). Table headers carry `listSlot`s so rows collapse to a list on narrow screens; `paginate` + `onNextPage` drive the cursor. Action `intent=generateBatch` (`productIds[]`, `merchantContext`, `model`, `idempotencyKey`): queues one AI generation per selected product (max 20) through `startBatchGeneration`; rate-limited with the admin write bucket; result banner lists skipped products |
