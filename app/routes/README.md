...
| `app.products._index.tsx` | `/app/products` | List. Filters from URL: `query`, `status`, `hasBadge`, `after` (keyset cursor = last local id). Unknown values are ignored. Action `intent=generateBatch` (`productIds[]`, `merchantContext`, `idempotencyKey`): queues one AI generation per selected product (max 20) through `startBatchGeneration`; rate-limited with the admin write bucket; result banner lists skipped products |
