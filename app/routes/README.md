...
| `app.products._index.tsx` | `/app/products` | List. Filters from URL: `query`, `status`, `hasBadge`, `after` (keyset cursor = last local id). Unknown values are ignored. Loader also sends the allowed model names. Action `intent=generateBatch` (`productIds[]`, `merchantContext`, `model`, `idempotencyKey`): queues one AI generation per selected product (max 20) through `startBatchGeneration`; rate-limited with the admin write bucket; result banner lists skipped products |
