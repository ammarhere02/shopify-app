...
5. Write for the product's category. Work it out from the title, product type, tags and images, then cover what a shopper in that category wants to know:
   ...
   When RESEARCHED_FACTS are given, use the most useful of them: they are verified specifications for this product.
...
8. descriptionHtml may use only these tags, without attributes: ${ALLOWED_TAGS.join(", ")}. Aim for 80-150 words: an opening paragraph of two or three sentences on what the product is and what sets it apart, then a <ul> of 4-6 supported features, each a concrete fact with a short note on why it matters, when the sources give at least two. No headings. Every sentence must say something true and specific about this product; cut any sentence that does not.
...
2. Identify the product before searching. Look closely at the images for the brand: logos, emblems, signature design marks and printed labels (for example a trefoil logo with three shoulder stripes is adidas Originals). The vendor field may be the store's own name rather than the maker: when the images show a brand, search for that brand, not the vendor. Combine the brand with the title, type, colour and visible design details, for example "adidas Originals 3-Stripes T-shirt green". If PRODUCT_DATA has SKUs that look like a brand style code (letters and digits, e.g. "FT5176-00L-BLK"), search for the brand plus that code FIRST: it is the most reliable way to find the exact item. Otherwise search for the exact product (brand plus model or full product name). Run at most the allowed number of searches; stop as soon as you have official specifications or it is clear the product cannot be found. After your last allowed search you MUST write the final JSON answer, even if nothing was found: never end without it.
...
4. Decide "identified" like this:
   - Products with a model name (phones, electronics, appliances, shoes, named product lines): "identified" is true when a result names the same brand, model and generation as this product, e.g. "Samsung Galaxy Z Flip6" for a product titled "Samsung Galaxy Z Flip 6". Colour, storage and size variants of that model ARE this product; their shared specifications apply. Report a specification that differs by variant (storage, colour, size) only when the variant matches this product.
   - Products without a model name (plain clothing, generic goods): "identified" is true only when a result has the same SKU or style code, or the same named product with the same design as the images.
   - A different generation or edition (Flip 5 vs Flip 6), a different model, a similar product or a generic match is NOT this product: set "identified" to false and leave "facts" empty.
   Be consistent: if your notes say the sources match this product, "identified" must be true with "high" or "medium" confidence and the specifications must be in "facts".
...
export function buildResearchMessages(input: {
  ...
  const text = [
    ...
    block("MERCHANT_FACTS", merchantContext),
    block("IMAGE_ALT_TEXT", images.map((image, i) => ({ image: i + 1, alt: image.alt }))),
    `${images.length} product image(s) follow. You may run at most ${input.maxSearches} search(es). Find the exact product, then always finish with the JSON answer.`,
  ].join("\n\n");
  return [
