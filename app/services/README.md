...
| `generation-state.ts` | `canMoveJob`, `canMoveReview`, `canHaveReview`, `canEditDraft`, `isAbandoned`, `JOB_ABANDON_MS` (5 min) | Pure. The two state fields of a description generation: job `status` (QUEUED → RUNNING → SUCCEEDED/FAILED) and merchant `reviewStatus` (DRAFT → APPROVED/REJECTED, APPROVED → APPLIED or back to DRAFT when the Shopify write is refused). Repositories turn these into conditional updates |
...
| `description-generation.server.ts` | `startGeneration(deps, shopId, input)`, `runGeneration`, `fetchProductForDescription`, `selectableImages`, `createGenerationDeps(adminGraphql, logContext)`, `GenerationError` (VALIDATION / NOT_FOUND / LIMIT / CONFLICT) | AI description generation, see below |
...
| `generation-api.server.ts` | `withGenerationErrors`, `toApiError`, `apiGenerationDeps(shop, requestId)`, `parseJobId`, `idempotencyKeyFrom`, `generationFields`, `asObject` | `/api/v1` glue. `GenerationError` → 422 `validation_failed` / 404 `not_found` / 429 `generation_limit_reached` / 409 `invalid_state`; `AiConfigError` → 503 `ai_not_configured`; no offline session → 409 `shop_session_unavailable` |
