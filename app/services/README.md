...
| `description-prompt.ts` | `buildDescriptionMessages`, `trustedText`, `PROMPT_VERSION` (`v1`), `MERCHANT_CONTEXT_MAX` (2000) | Pure. System message = rules only. Shopify fields, merchant facts and image alt text are JSON-encoded inside labelled untrusted-data blocks, so input text cannot close a block or pose as a rule. Text part first, then `image_url` parts. Bump `PROMPT_VERSION` when the wording changes |
| `description-output.ts` | `DESCRIPTION_JSON_SCHEMA`, `validateModelOutput`, `detectUnsupportedClaims`, `mergeWarnings`, `OUTPUT_LIMITS` | Pure. The schema sent to the provider and the same rules re-checked on the server: JSON object, no extra or missing fields, types, lengths (HTML 10,000; short 300; SEO 70/160; 8 highlights × 120). Unwraps a Markdown fence. Sanitizes `descriptionHtml`, flattens the plain-text fields, keeps `raw` untouched. Claim detection: 8 categories (medical, legal, sustainability, certification, origin, performance, warranty, material); a phrase found in the generated text but not in the trusted text becomes a warning |
