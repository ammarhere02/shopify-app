...
## Verify and document
Run and report the real output counts of `npm test`, `npm run test:integration`, `npm run typecheck`, `npm run lint`. Update the table in `tests/README.md` for new or extended files. Do not tick `docs/VERIFICATION.md` items that need live dev-store evidence. Do not `git commit` or push.
