...
| `AI_RESEARCH`, `AI_RESEARCH_MAX_SEARCHES`, `AI_RESEARCH_MAX_RESULTS` | Web research before each description: `on` (default) or `off`; searches per generation (default 2, max 5); results per search (default 5). Each search is billed by OpenRouter on top of the tokens |
