...
13. **Repair what is only displayed, reject what is written.** The first live run (Gemini 2.5 Flash) returned a good description with a 182-character `seoDescription` and the job failed on the 160 limit, although the prompt stated it: models do not count characters reliably. `descriptionHtml` is the only field this app writes to Shopify, so it stays strict (over the limit = failed job; HTML is never truncated). The other fields are suggestions shown to the merchant, so a modest overrun is cut at a word boundary and listed as a warning, the model's original text stays in `raw_json`, and an answer beyond five times a limit still fails. The prompt (`v2`) now asks for targets well under each limit.
...
17. **An in-flight state, not a lock.** `APPLYING` exists so a process death between the mutation and the version row is visible instead of silently looking like "never applied". A job left in `APPLYING` for over two minutes goes back to `APPROVED` with an error note telling the merchant to check the product and apply again; the retry then meets decision 9's exception and records the version. Nothing tries to undo a write in Shopify.
18. **Publish needs an ACTIVE product and a real channel.** `publishablePublish` accepts a DRAFT product and shows nothing, so the service refuses with 409 before calling Shopify. The channel id must be a `Publication` GID and one of the shop's own app catalogs (read live), so a client cannot pass an arbitrary id through. The audit row is written as `REQUESTED` before the call and completed after, so an attempt is on record even if the process dies mid-call.
19. **Restore is a new write, and the original text is restorable.** A version row keeps both what it wrote and what it replaced. "Restore" writes an older row's text through the Apply path and appends a `RESTORE` row pointing at its source; "Restore what it replaced" on the newest row brings back the description the merchant had before this app touched the product. History is append-only: nothing is edited or deleted.
...
20. **Hardening pass (2026-09-22).** Three gaps found by auditing the §12 list against the code, all fixed: (a) the logger redacted by key name only, so a Shopify or provider error message quoting a URL with a key, or a base64 image inside a "reason", would have been logged; values are now scanned for credential shapes and `data:` URIs and cut at 500 characters at every depth. (b) A Shopify outage during apply/publish surfaced as 500 `internal_error`, which reads as our bug and hides whether a retry makes sense; it is now 429/502/409 with a code per kind and a fixed sentence (Shopify's text stays in the log). (c) The admin page had no request limit at all: session auth carries no API-key bucket, so a script with a valid session token could start generations at will; write intents now share a 10/min per-shop bucket. Verified as already holding: sanitizer at output, edit and pre-mutation; limits counted in MySQL; idempotent generate; single-winner apply; uniform API envelope.
