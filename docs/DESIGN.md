...
22. **Research is a second, separate call whose facts must be confirmed, not believed.** A vision model given only a title and photos guesses specifications. So a generation now makes two calls. The first is text-only research with OpenRouter's `openrouter:web_search` server tool (bounded by `max_uses`, `max_results` and `max_tool_calls`, so the cost per generation is capped): find the exact product from its title, vendor, type, tags and merchant facts, and return specifications each with a source URL. Images are deliberately not sent: a photo can suggest a category, never an exact model. The second call is the existing description prompt, given a `RESEARCHED_FACTS` block. Between them stands a validator that does not trust the model's own claims: a fact is used only when the model reports the exact product identified with medium or high confidence AND the fact's source host is among the pages OpenRouter's search actually returned (its `url_citation` annotations). No citations means no facts. Anything else ends as `UNCERTAIN` with nothing used and a warning next to the draft; a failed research call ends as `FAILED` the same way, and the description is still written from the Shopify data. The record (facts, rejected facts with reasons, cited pages, search count) is stored inside the existing `validated_json` column (no migration; older generations read `research: null`) and shown on a **Sources** tab so the merchant can open each page before approving. Job states, idempotency, the input hash, privacy settings (same `data_collection` policy on both calls) and the spend limits are unchanged: one job, one daily-limit count. Products without a vendor or a model-like token in the title are not researched, because a search cannot identify "Blue Beanie".
...
**Stored:** product snapshot, merchant context, selected media IDs, raw and validated model JSON, the research record (facts used, facts rejected with reasons, cited page URLs and titles, search count), warnings, model, prompt version, input hash, token usage, estimated cost and latency (both calls summed), OpenRouter generation id, error summary, every applied description with the value it replaced, publish actions with their `userErrors`.
