...
- [x] Two calls per generation: research (product data, SKUs and the selected images, OpenRouter `openrouter:web_search` server tool bounded by `AI_RESEARCH_MAX_SEARCHES` / `AI_RESEARCH_MAX_RESULTS`, `max_tool_calls`) then the description with a `RESEARCHED_FACTS` block (prompt `v6`); a failed research call shows the provider's exact message (finish reason, output tokens) to the merchant
